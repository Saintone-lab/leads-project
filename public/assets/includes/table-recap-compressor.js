$(function () {
    var dt_table_recap_compressor = $(".datatable-recap-compressor");
    var Url = "/db/recap-compressor/";
    var path = window.location.pathname;
    var date = path.substring(path.lastIndexOf("/") + 1);

    // console.log("ID:", id); // Output: ID: 2

    if (dt_table_recap_compressor.length) {
        $('[data-toggle="tooltip"]').tooltip();
        var dt_product = dt_table_recap_compressor.DataTable({
            ajax: {
                type: "GET",
                url: Url + date,
                headers: {
    ...
    dt_table_recap_compressor.on("draw", function () {
        $('[data-toggle="tooltip"]').tooltip();
    });
});
