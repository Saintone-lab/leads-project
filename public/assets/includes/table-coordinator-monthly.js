$(function () {
    var dt_table_monthly = $(".datatable-monitoring-monthly");
    var Url = "/db/service-manager/monthly/";
    var path = window.location.pathname;
    var id = path.substring(path.lastIndexOf("/") + 1);

    // console.log("ID:", id); // Output: ID: 2

    if (dt_table_monthly.length) {
        $('[data-toggle="tooltip"]').tooltip();
        var dt_product = dt_table_monthly.DataTable({
            ajax: {
                type: "GET",
                url: Url + id,
                headers: {
    ...
    dt_table_monthly.on("draw", function () {
        $('[data-toggle="tooltip"]').tooltip();
    });
});
